...
def strip_think_blocks(agent, content: str) -> str:
    ...
    #
    #    If the post-tag text contains a clear ``\n\n`` paragraph
    #    break followed by a non-empty answer tail, the model emitted
    #    thinking, a blank line, then the actual response.  Strip
    #    only the open tag and the thinking paragraph so the answer
    #    survives into the user-visible content.  Without this split,
    #    M2.7's "thinking...answer with no close tag" pattern would
    #    produce an empty response and a populated reasoning box —
    #    the user would see the response panel blank while reasoning
    ...
    def _strip_unterminated(match: "re.Match[str]") -> str:
        body = match.group("body")
        _thinking, _sep, _trailing = body.partition("\n\n")
        if _sep and _trailing.strip():
            # Keep the answer tail, drop the open tag + thinking paragraph.
            return "\n" + _trailing.lstrip("\n")
        # No paragraph break — strip the whole post-tag run.
        return ""

    content = _unterm_body_pat.sub(_strip_unterminated, content)
...
def extract_reasoning(agent, assistant_message) -> Optional[str]:
    ...
    if not reasoning_parts and isinstance(content, list):
        ...
        # Some providers (and OpenRouter in some configurations) use
        # ``type: "reasoning"`` instead of ``type: "thinking"``.  Both
        # carry chain-of-thought that must be captured into reasoning
        # so it never leaks into user-visible content.  Refs #21944.
        for block in content:
            if not isinstance(block, dict):
                continue
            btype = block.get("type", "")
            if btype not in ("thinking", "reasoning"):
                continue
            thinking_text = (
                block.get("thinking")
                or block.get("text")
    ...
    if not reasoning_parts and isinstance(content, str) and content:
        ...
        # ── Unterminated blocks ─────────────────────────────────────────────
        # MiniMax M2.7 (and other providers) send <think> without a
        # closing .  The non-greedy closed-block patterns above return an
        # empty capture for these (the engine matches the open tag then
        # backtracks forever looking for  which never arrives), so
        # unterminated content is silently lost.  Case 2 of strip_think_blocks
        ...
        #
        # Only match unterminated blocks where the tag is at the START of
        # the content string AND there is NO close tag anywhere in it.
        ...
        #
        # When the unterminated content has a clear ``\n\n`` paragraph
        # break, split there: the first paragraph is reasoning, the rest
        # is a candidate answer that flows back into ``content`` so the
        # user sees the actual response.  Without the split, stripping
        # the open tag to end-of-string eats the answer (refs the
        ...
        if not reasoning_parts:
            ...
            for m in _unterm_pat.finditer(content):
                ...
                has_close = any(
                    re.search(pat, after_open, flags=re.IGNORECASE)
                    for pat in (
                        r'</think>', r'</think', r'</reasoning>',
                        r'</thought>', r'</REASONING_SCRATCHPAD>',
                    )
                )
                if has_close or not after_open.strip():
                    continue
                thinking_text, sep, trailing_answer = after_open.partition("\n\n")
                if sep and trailing_answer.strip():
                    # Leave the answer tail in content; capture only the
                    # thinking paragraph.
                    content = trailing_answer
                    reasoning_parts.append(thinking_text.strip())
                else:
                    reasoning_parts.append(after_open.strip())
                break  # only one unterminated block at content start
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # MiniMax on its OpenAI-compatible /v1 endpoint (chat_completions mode).
    # MiniMax documents passive auto-caching on /v1/chat/completions: the
    # server reads cache_control markers on inner content parts (envelope
    # layout, native_anthropic=False) and returns cached prompt bytes at
    # 0.1x read pricing without the client needing to opt in explicitly.
    # Without this branch, provider=minimax / minimax-cn on /v1 falls
    # through to (False, False) and every request re-bills the full
    # prompt — burning through compute windows on long conversations.
    # Docs: https://platform.minimax.io/docs/api-reference/anthropic-api-compatible-cache
    if not is_anthropic_wire:
        is_minimax_provider = provider_lower in {"minimax", "minimax-cn"}
        is_minimax_host = (
            base_url_host_matches(eff_base_url, "api.minimax.io")
            or base_url_host_matches(eff_base_url, "api.minimaxi.com")
