...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # ── List-typed content normalise ─────────────────────────────────────
    # MiniMax M3, DeepSeek v4, Kimi, and several other providers send
    # content as a list of typed blocks, e.g.
    #   [{"type": "thinking", "thinking": "..."},
    #    {"type": "text",      "text":      "..."}]
    # Reasoning blocks in the list were already captured into
    # ``reasoning_text`` above by ``_extract_reasoning`` (it iterates the
    # list and pulls every block of type "thinking"/"reasoning").  What
    # remains to do here is collapse the non-reasoning blocks into a
    # single string so the rest of this function can treat ``content``
    # uniformly without TypeError-ing on the string-only regex passes
    # (e.g. ``re.findall`` on a list, ``_sanitize_surrogates`` on a
    # list).  Without this normalisation, list-typed responses crashed
    # before reasoning was ever surfaced to the user — the regression
    # that caused "reasoning appears after the message" on M3.
    _raw_content = assistant_message.content or ""
    if isinstance(_raw_content, list):
        _text_parts: List[str] = []
        for _block in _raw_content:
            if not isinstance(_block, dict):
                _text_parts.append(str(_block))
                continue
            _btype = _block.get("type", "")
            if _btype in ("thinking", "reasoning"):
                # Captured into reasoning_text above; skip so it never
                # leaks into user-visible content.
                continue
            _text = _block.get("text") or _block.get("content") or ""
            if _text:
                _text_parts.append(_text)
        content = "\n".join(_text_parts)
    else:
    ...
    if not reasoning_text and isinstance(content, str) and content:
        # Extract ALL think block formats that _strip_think_blocks handles — not just
        # <think>.  This keeps extraction aligned with stripping so that reasoning
        # is captured into msg["reasoning"] rather than being silently stripped and
        # left to appear inline in the response content.
        inline_patterns = (
            r"<think>(.*?)</think>",
            r"<thinking>(.*?)</thinking>",
            r"<thought>(.*?)</thought>",
        ...
        for pat in inline_patterns:
            for m in re.findall(pat, content, flags=re.DOTALL | re.IGNORECASE):
                if m.strip():
                    think_blocks.append(m.strip())

        # ── Unterminated blocks ─────────────────────────────────────────────
        # MiniMax M2.7 (and some other providers) send <think> without a
        # closing .  The non-greedy closed-block patterns above return
        # an empty capture for these (the engine matches the open tag then
        # backtracks forever looking for  which never arrives), so
        # unterminated content is silently lost.  Case 2 of _strip_think_blocks
        # strips from the open tag to end-of-string — we mirror that here so
        # extraction is aligned with stripping and no reasoning is lost.
        #
        # Only match unterminated blocks where the thinking is at the START
        # of the content AND there is NO close tag anywhere in the string.
        # Properly closed blocks (even with empty content) are handled by
        # the closed-block patterns above and must NOT be double-extracted.
        #
        # The model often emits thinking, a paragraph break, then the
        # actual answer with no close tag, e.g.
        #   "<think>Let me work this out\n\nThe answer is 42."
        # Stripping the open tag to end-of-string would eat the answer
        # along with the thinking — the user would see only reasoning
        # and a blank response.  Split on the first ``\n\n`` and use the
        # first chunk as reasoning, the rest as a candidate answer that
        ...
        for m in _unterm_pat.finditer(content):
            ...
            # Only treat as unterminated if:
            # 1. There is NO close tag anywhere in the content, AND
            # 2. The content after the open tag does NOT contain a close tag
            # This prevents properly closed blocks from being double-extracted.
            _lt = chr(60)   # <
            _slash = chr(47)  # /
            _close_tags = tuple(
                f"{_lt}{_slash}{tag}{chr(62)}"
                for tag in ("think", "thinking", "reasoning", "thought", "REASONING_SCRATCHPAD")
            )
            _has_close_in_content = any(close in content for close in _close_tags)
            _has_close_in_after = any(close in after_open for close in _close_tags)
            if not _has_close_in_content and not _has_close_in_after and after_open.strip():
                thinking_text, sep, trailing_answer = after_open.partition("\n\n")
                if sep and trailing_answer.strip():
                    # Unterminated thinking followed by a clear paragraph
                    # break and a non-empty answer tail.  Capture only
                    # the thinking portion into reasoning; leave the
                    # answer tail in content so the user sees the actual
                    # response.  We do this by splicing ``content`` to
                    # drop the open tag + first paragraph, keeping
                    # everything from the first \n\n onward as content.
                    content = trailing_answer
                    think_blocks.append(thinking_text.strip())
                else:
                    think_blocks.append(after_open.strip())
    ...
    # ``content`` has already been normalised to a plain string above (list-typed
    # provider responses have their non-reasoning blocks joined into a single
    # string), so we can pass it straight to the string-only sanitiser without
    # hitting the TypeError that used to fire on list-typed content (refs #21944).
    _san_content = _sanitize_surrogates(content)
    if reasoning_text:
        reasoning_text = _sanitize_surrogates(reasoning_text)

    # Strip inline reasoning tags (<think>… etc.) from the stored
