...
# ── $dollar-prefixed skill/bundle invocation ──────────────────────────────
#
# The `$` token is a codex-style skill reference: `$skill-a` is an inline
# reference the user drops into a prompt (mid-prose OR position-0), and
# `$skill-a $skill-b do XYZ` is a forced multi-skill chain — every `$token`
# becomes a leading `/token` so the existing stacked-skill machinery carries
# the rest. Bundles (`$/bundle-name`) resolve the same way and take precedence
# over skills on name collision (mirrors `gateway/run.py:10421`).
#
# Token boundary regex mirrors the inline-`/` trigger at
# `ui-tui/src/domain/slash.ts:31`: letter-start, word-end at whitespace /
# EOL, leading `$` must sit at start-of-text or after whitespace — so a
# path-shaped `$PATH` does not match.

_DOLLAR_TOKEN_RE = re.compile(r"(?:^|\s)\$([a-zA-Z][\w-]*)")


def _resolve_dollar_token(token: str) -> Optional[str]:
    """Resolve a typed ``$token`` to its canonical ``/slug`` key.

    Returns the canonical key (``"/skill-name"``) when *token* matches an
    installed bundle (preferred) or skill. Returns ``None`` when unknown
    or disabled.

    Underscores are interchangeable with hyphens — same rule the
    ``/``-prefixed dispatcher uses for Telegram bot-command compat.
    """
    if not token:
        return None
...
def find_dollar_refs(text: str) -> list[tuple[str, str]]:
    """Return ``(kind, /slug)`` tuples for every ``$ref`` in *text*.

    *kind* is ``"bundle"`` or ``"skill"``. Bundles are checked first; a
    ``$name`` that matches a bundle is reported as a bundle even if a
    skill of the same name exists. Order is preserved (first match first).
    Unknown or disabled names are silently dropped.

    Empty input or input without ``$tokens`` returns an empty list.
    """
    if not text:
        return []
    out: list[tuple[str, str]] = []
    seen: set[str] = set()
    # Resolve which kinds live at which key once per call, instead of
    ...
    for match in _DOLLAR_TOKEN_RE.finditer(text):
        token = match.group(1)
        key = "/" + token.lower().replace("_", "-")
        if key in seen:
            continue
        if key in bundles:
            seen.add(key)
            out.append(("bundle", key))
        elif key in skills:
            seen.add(key)
            out.append(("skill", key))
    return out


def rewrite_dollar_chain_to_slash(text: str) -> tuple[str, list[tuple[str, str]]]:
    """Rewrite ``$a $b do XYZ`` at position 0 to ``/a /b do XYZ``.

    Returns ``(rewritten_text, refs)``. ``refs`` is the same shape as
    :func:`find_dollar_refs` — ``(kind, /slug)`` tuples in order.

    Mid-prose ``$tokens`` are NOT rewritten here — those are handled by
    the TUI/Desktop completion adapter as inline references. The CLI
    rewrite is the command-style trigger only, so a prompt like
    ``please run $skill-a inline`` passes through unchanged.

    Unknown ``$tokens`` at position 0 are silently dropped (the user
    gets no error, the rewrite just omits the unknown token). Disabled
    tokens are also dropped (mirroring :func:`find_dollar_refs`).
    """
    if not text or not text.startswith("$"):
        return text, []
    refs = find_dollar_refs(text)
    if not refs:
        # No known $refs at position 0 — return text unchanged so the
        # downstream handler treats it as ordinary prose.
        return text, []
    # Walk through the original text, replacing each `$token` that
    # resolved with `/slug`. We tokenize on whitespace so we don't depend
    # on the regex's exact match positions. Leading `$tokens` that
    # resolved become `/slug`; `$tokens` whose name is unknown are
    # silently dropped (the forgiving stance :func:`find_dollar_refs`
    # takes — the user sees no error, the rewrite just omits them). Plain
    ...
    for tok in tokens:
        if tok.startswith("$") and len(tok) > 1:
            token_name = tok[1:]
            key = _resolve_dollar_token(token_name)
            if key is not None:
                rewritten_tokens.append(key)
            # Unknown $token — drop it (don't keep the bare `$name` in
            # the rewritten text; that would leak an unresolvable token
            # into the agent turn).
            continue
        # Not a $token — pass through unchanged.
        rewritten_tokens.append(tok)
    rewritten = " ".join(rewritten_tokens)
    return rewritten, refs


def build_dollar_invocation_message(
    text: str,
    user_instruction: str = "",
    task_id: str | None = None,
) -> Optional[tuple[str, list[str], list[str]]]:
    """Build the user message for a ``$ref``-prefixed prompt.

    Resolves ``$tokens`` in *text* via :func:`find_dollar_refs`. If any
    bundle is present, the bundle's invocation message is the leading
    scaffold; leftover skill ``$tokens`` become a stacked-skill scaffold
    after. When only skills are present, delegates to
    :func:`build_stacked_skill_invocation_message` directly so the
    scaffold markers (``[IMPORTANT: ... skill bundle, ...]``) are
    byte-identical to the stacked-skill path — that lets
    :func:`extract_user_instruction_from_skill_message` recover the
    user's instruction without new marker plumbing.

    If *user_instruction* is empty, the caller-supplied text after the
    ``$tokens`` is used (i.e. ``$skill-a $skill-b do XYZ`` ⇒ the
    trailing ``do XYZ`` becomes the user instruction).

    Returns ``(message, loaded_skill_names, missing)`` or ``None`` when
    no ``$ref`` resolves to an installed skill or bundle.
    """
    refs = find_dollar_refs(text)
    if not refs:
        return None

    bundle_ref = next((key for kind, key in refs if kind == "bundle"), None)
    skill_keys = [key for kind, key in refs if kind == "skill"]

    # When the caller doesn't supply an explicit user_instruction, derive
    # one from the text by stripping the leading $tokens. The result is
    # the prose the user wants the agent to act on after the skills load.
    if not user_instruction:
        # Drop every `$token` (resolved OR unknown) from the leading
        # portion. We tokenize so we don't depend on the regex's match
        # positions. The remaining prose is the user instruction.
        kept: list[str] = []
        for tok in (text or "").split():
            if tok.startswith("$") and len(tok) > 1:
                # Drop ANY $token at position 0 — the user typed a
                # `$chain` so the dollar-tokens are command syntax, not
                # prose the agent should see.
                continue
            kept.append(tok)
        user_instruction = " ".join(kept)

    if bundle_ref is not None:
        # Bundle wins on collision. Route through the existing bundle
        # loader so per-platform disabled gating + missing-skill notes
        # behave identically to a `/bundle-name` invocation.
        try:
            from agent.skill_bundles import build_bundle_invocation_message as _build_bundle
            bundle_result = _build_bundle(bundle_ref, user_instruction, task_id=task_id)
        except Exception:
            bundle_result = None
        ...
        if skill_keys:
            # Mixed: append a stacked-skill scaffold for the leftover
            # $skills. Each skill body is appended after the bundle
            # scaffold so the agent sees both layers in order.
            stacked_result = build_stacked_skill_invocation_message(
                skill_keys, user_instruction, task_id=task_id
            )
            if stacked_result is not None:
                stacked_msg, stacked_loaded, stacked_missing = stacked_result
                msg = f"{msg}\n\n{stacked_msg}"
                loaded = loaded + stacked_loaded
                missing = missing + stacked_missing
        return msg, loaded, missing

    # Skills-only path — same scaffold markers as the existing
