...
from agent.conversation_compression import conversation_history_after_compression, recover_rotated_compression_session
...
def build_turn_context(
    ...
    *,
    ...
    install_safe_stdio()
    # Rule-1 catchup fix: recover a session rotated by another path before
    # binding log/turn ids or copying client-supplied history. Block ported verbatim
    # from upstream commit 0ee8d4187 ("fix(compression): recover rotated session
    # lineage"). The ours-wins catchup merge dropped this call site; restoring
    # it preserves the invariant that the produced TurnContext.history reflects
    # the canonical child after a session-id rotation in another path.
    ...
    # Rule-1 catchup fix: tell auxiliary_client what the live main provider/model
    # are for this turn after primary restoration has settled the runtime.
    # Block ported verbatim from upstream commit 0ee8d4187.
    try:
...
# === Rule-1 catchup fix: more upstream-only additions ===
