...
import { inlineDollarTrigger, inlineSlashTrigger, looksLikeSlashCommand } from '../domain/slash.js'
...
export function completionRequestForInput(
  ...
  | { method: 'complete.slash'; params: { text: string; skillsOnly?: boolean; dollarOnly?: boolean }; replaceFrom: number }
  | null {
  ...
  if (inline) {
    return {
      ...
      params: { text: `/${inline.query}`, skillsOnly: true },
      replaceFrom: inline.start + 1
    }
  }

  // A `$token` mid-message OR at position 0 is a skill/bundle reference.
  // Unlike `/`, `$` is allowed at position 0 (forced multi-skill chain) and
  // is never path-shaped, so it doesn't conflict with TAB_PATH_RE. The
  // completion adapter (`useCompletion`) filters results to skills+bundles
  // via the `dollarOnly` flag passed to `complete.slash`.
  const dollar = inlineDollarTrigger(input)

  if (dollar) {
    return {
      method: 'complete.slash',
      params: { text: `/${dollar.query}`, dollarOnly: true },
      replaceFrom: dollar.start + 1
    }
  }
    ...
    const t = setTimeout(() => {
      ...
      gw.request<CompletionResponse>(request.method, request.params)
        .then(raw => {
          ...
          // Mid-message slash references: trust the gateway's prefix match.
          // The existing `kind === 'skill'` filter was always matching nothing
          // because `complete.slash` doesn't tag items with `kind` — a pre-
          // existing latent bug that broke mid-prose skill autoload the same
          // way `$`-autoload was broken. Drop the filter; the gateway's
          // prefix match is sufficient (skill slugs are unique enough).
          const items =
          ...
          const slashParams = request.method === 'complete.slash' ? request.params : null
          setCompReplace(
            slashParams && !slashParams.skillsOnly && !slashParams.dollarOnly
              ? (r?.replace_from ?? 1)
              : request.replaceFrom
          )
        })
