...
class GatewaySlashCommandsMixin:
    ...
    def _clear_conversation_scope(self, session_key: str, reason: str) -> None:
        """Clear every conversation-scoped per-session dict for session_key.

        This is the single funnel through which all conversation-boundary
        resets (/new, /resume, auto-reset, expiry finalization,
        compression-exhausted reset) must pass.  It prevents the
        boundary-drift class of bug where a new dict is added to the
        runner but one or more boundary sites forget to pop it.
        Silently succeeds when a dict does not exist or the key is absent.
        """
        # Guard: empty key is a degenerate no-op, not an error.
        if not session_key:
            return
        ...
        # Boundary security state is also per-session and cleared here so
        # the two reset sites do not each need to maintain a parallel list.
        # Use getattr guard — the runner may be a bare object with no attrs
        # initialized (e.g. test_funnel_is_bare_runner_safe_and_empty_key_noop).
        for _attr in ("_pending_approvals", "_update_prompt_pending", "_pending_skills_reload_notes"):
