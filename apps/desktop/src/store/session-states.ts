...
// When the user switches to a non-tile session via sidebar click, apply that
// session's persisted token counts to $currentUsage so the pill shows the
// newly-selected session's usage instead of the previously-active session's.
$selectedStoredSessionId.listen(selected => {
  if (!selected) return

  // Check if this stored session has an open tile — if so, the
  // $focusedRuntimeId listener handles $currentUsage separately.
  const hasOpenTile = $sessionTiles.get().some(t => t.storedSessionId === selected)
  if (hasOpenTile) return

  // No tile: apply the stored session's persisted tokens to $currentUsage.
  // This covers sidebar clicks to sessions not currently open in a tile.
  const stored = $sessions.get().find(s => s.id === selected)
  if (stored) {
    const input = stored.input_tokens || 0
    const output = stored.output_tokens || 0
    $currentUsage.set({ ...$currentUsage.get(), input, output, total: input + output })
  }
})

// When focused runtime changes (tile focus switch), sync $currentUsage from
// that tile's per-session state so the pill reflects the focused tile's usage.
$focusedRuntimeId.listen(runtimeId => {
  if (!runtimeId) return
  const focused = $focusedSessionState.get()
  if (focused?.usage) {
    $currentUsage.set({ ...$currentUsage.get(), ...focused.usage })
  }
})
