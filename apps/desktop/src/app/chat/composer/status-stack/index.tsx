...
export function ComposerStatusStack({ queue, sessionId }: ComposerStatusStackProps) {
  ...
  // Resolve the runtime id the parent passes to the durable STORED id the
  // store actually keys by. A visibility-change reconnect (use-gateway-boot)
  // can rotate the runtime id under us; reading by stored id keeps the panel
  // anchored to the same chat across that rotation. Background-process
  // refresh below still uses the runtime id — that registry is genuinely
  // socket-scoped, not lineage-scoped.
  const todoKey = sessionId ? (sessionStates[sessionId]?.storedSessionId ?? sessionId) : null

  const groups = useMemo(
    () => groupStatusItems(todoKey ? (itemsBySession[todoKey] ?? []) : []),
    [itemsBySession, todoKey]
  )
