...
export function useSlashCompletions(options: {
  ...
  const fetcher = useCallback(
    async (query: string): Promise<CompletionPayload> => {
      ...
      // If the query starts with `$`, transform to `/${rest}` for the existing
      // RPC, then map completion items back to `$rest` on insertion. The
      // gateway's `complete.slash` filters results to skills+bundles when
      // the request includes `dollarOnly: true` (matches the TUI).
      const isDollarQuery = query.startsWith('$')
      const dollarOnly = isDollarQuery
      const text = isDollarQuery ? `/${query.slice(1)}` : `/${query}`
      ...
      try {
        ...
        const decorated = (result.items ?? [])
          .map(item => {
            // Dollar-query items come back as `/skill-a`; rewrite to `$skill-a`
            // so the composer's `complete.slash` text on submit stays in
            // the dollar form until the user submits, at which point the
            // dispatch layer rewrites to `/a /b …`.
            if (dollarOnly && !isArgCompletion && typeof item.text === 'string' && item.text.startsWith('/')) {
              return { ...item, text: `$${item.text.slice(1)}` }
            }
            if (!isArgCompletion) {
              return item
            }
