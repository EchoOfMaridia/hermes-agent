    ...
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      if (warmHit) {
        ...
        if (sessionShouldHaveTranscript(stored) && cachedViewState.messages.length === 0) {
          ...
          //
          // Profile-resolution defense: the session row's `profile_name` is
          // NULL on disk for most sessions, but `resolveStoredSession` may
          // resolve `sessionProfile` to a git-branch-named pseudo-profile
          // (e.g. `catchup-upstream-2026-07-26`, `simulation-mode`). A pool
          // backend for those pseudo-profiles is hard to spawn — the Electron
          // main process can fail with "exited before it became ready (1)".
          // When that happens the profile-specific fetch returns an empty
          // transcript. Try the default-profile fetch as a fallback so the
          // chat thread still loads when the actual session is on `default`.
          const primaryTranscriptPromise = isWatchWindow()
            ? null
            : getSessionMessages(storedSessionId, sessionProfile).catch(() => null)
          const defaultProfileFallbackPromise =
            isWatchWindow() || !sessionProfile || sessionProfile === 'default'
              ? null
          ...
          // On a warm cached resume (session was already active) the session start
          // time is unchanged — preserve it so the statusbar clock doesn't reset.
          // Only mint a fresh timestamp when this is a genuinely new runtime id.
          if ($sessionStartedAt.get() === null || $sessionStartedAt.get() === undefined) {
            setSessionStartedAt(Date.now())
          }
          ...
          try {
            ...
            if (activated.session_key && activated.session_key !== storedSessionId) {
              ...
              if (!running && primaryTranscriptPromise) {
                let persisted = await primaryTranscriptPromise

                if (!isCurrentResume()) {
                  return
                }

                // The profile-specific fetch may have failed because the
                // pool backend for a branch-named pseudo-profile is wedged.
                // Fall back to the default-profile fetch (which is where the
                // session row actually lives on disk for most sessions).
                if ((!persisted || !persisted.messages?.length) && defaultProfileFallbackPromise) {
                  const fallback = await defaultProfileFallbackPromise
      ...
      try {
        ...
        // Defensive fallback: if BOTH the prefetch and the resume RPC returned
        // empty messages but the session row reports a non-zero `message_count`,
        // the per-profile pool backend is almost certainly wedged (a session
        // whose `profile_name` is NULL on disk but resolves to a git-branch-named
        // pseudo-profile can fail to spawn a pool backend — and the cache returns
        // an empty transcript instead of falling back to the default profile).
        // Try a default-profile fetch once before latching the resume failure.
        let fallbackMessages: typeof messagesForView | null = null
        if (
          ...
          try {
            ...
            const fallbackChat = toChatMessages(fallback.messages)
            if (fallbackChat.length > 0) {
              fallbackMessages = reconcileAuthoritativeMessages(fallback.messages, currentMessages)
            }
          } catch {
            // Best-effort: if the default-profile fetch also fails, fall through
            // to the original latch so the user sees the failure surface.
          }
        }

        const finalMessages = fallbackMessages ?? messagesForView
        ...
        if (sessionShouldHaveTranscript(stored) && finalMessages.length === 0) {
          setActiveSessionId(null)
          activeSessionIdRef.current = null
        ...
        if (!chatMessageArraysEquivalent($messages.get(), finalMessages)) {
