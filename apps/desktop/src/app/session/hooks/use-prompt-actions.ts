...
// Position-0 `$a` is the forced multi-skill chain affordance. Same shape
// as SLASH_COMMAND_RE (`/^[^/s]*(?:\s|$)/`) but with `$` as the trigger.
// Mirrors `ui-tui/src/hooks/useCompletion.ts:INLINE_DOLLAR_RE` and the
// Python helper `agent.skill_commands.find_dollar_refs`.
const DOLLAR_PREFIX_RE = /^\$[a-z][\w-]*(?:\s|$)/
...
export function usePromptActions({
  ...
  const submitText = useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      // Position-0 `$a $b do XYZ` chains are an extension of the slash
      // command surface — rewrite to `/a /b do XYZ` and route through
      // the existing slash dispatch path. Bundles (`$/bundle`) take the
      // same route. Mid-prose `$tokens` are inline references and pass
      // through unchanged as ordinary prompt text.
      if (!attachments.length && DOLLAR_PREFIX_RE.test(visibleText)) {
        const rewritten = rewriteDollarPrefixToSlash(visibleText)
        if (rewritten !== visibleText) {
          triggerHaptic('selection')
          await executeSlashCommand(rewritten)
          return true
        }
      }
