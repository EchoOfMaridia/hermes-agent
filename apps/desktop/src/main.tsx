...
// Check ?win=overlay for the pet overlay BrowserWindow (separate Electron window,
// same bundle).  window.name is set by Electron when creating the BrowserWindow,
// before loadURL, providing a reliable fallback when searchParams are unavailable.
const isOverlay =
  new URLSearchParams(window.location.search).get('win') === 'overlay' ||
  window.name === 'pet-overlay'
console.info('[main] isOverlay:', isOverlay)
if (isOverlay) {
