...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Bumped 600 -> 1800 (30 min) so CLI (config["clarify"]["timeout"])
        # and gateway agree on the wait window. Interactive-plan research
        # phases routinely exceed the old 10 min ceiling.
        "clarify_timeout": 1800,
    ...
    "delegation": {
        ...
        "max_iterations": 100,  # per-subagent iteration cap (each subagent gets its own budget,
    ...
    if env_path.exists():
        ...
        for line in lines:
            ...
            if line and not line.startswith('#') and '=' in line:
                key, _, value = line.partition('=')
                # Strip inline comments to match python-dotenv's default
                # behavior. Without this, a line like
                # ``MINIMAX_BASE_URL=https://x/v1  # override`` ends up with
                # the trailing comment baked into the value, which then
                # produces 404s when the URL is sent to the API. The
                # ``\s+#`` anchor preserves legitimate ``#`` chars in
                # values (e.g. URL fragments like ``#section``).
