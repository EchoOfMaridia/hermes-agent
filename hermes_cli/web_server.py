...
from hermes_cli.memory_providers import (
    ...
    DeclaredProviderField as DeclaredProviderField,
...
@_ttl_cache(ttl_seconds=2.0)
def _workflow_runtime_singleton():
    """Lazy WorkflowRuntime for the REST layer. The workflow plugin's
    register() builds a runtime and publishes it via the module-level
    `get_active_runtime()` seam — the REST path must use THAT runtime
    (its dispatcher chain is the one the desktop_event_bridge wraps,
    and the FallbackDispatchSink is wired there). Constructing a
    parallel runtime via `build_runtime()` here meant every
    REST-launched run had an unwired dispatcher and the Hermes
    desktop's Workflows panel never saw it.

    When the plugin hasn't been registered into a context (e.g. a
    `hermes serve` boot that hasn't called the plugin loader yet),
    fall back to building a runtime ourselves AND register the
    desktop_event_bridge on it so the panel still populates."""
    # Path A: prefer the plugin's already-registered runtime so the
    # desktop_event_bridge and FallbackDispatchSink wired in
    # plugins/hermes_workflow/__init__.py:register() are in effect.
    try:
        from plugins.hermes_workflow import get_active_runtime
        rt = get_active_runtime()
        if rt is not None:
            return rt
    except Exception as exc:
        _log.debug("workflow plugin get_active_runtime probe failed: %s", exc)

    # Path B: build a fresh runtime and wire the desktop_event_bridge
    # on it so REST-launched runs still flow to /api/ws subscribers
    # even when the plugin loader hasn't run.
    try:
        from plugins.hermes_workflow import runtime_factory
        rt = runtime_factory.build_runtime(
        ...
        try:
            ...
            get_bridge().register(rt)
        except Exception as exc:
            _log.debug("desktop_event_bridge register on REST runtime failed: %s", exc)
        return rt
    except Exception as exc:
...
async def workflows_run(request: Request, body: dict):
    ...
    # Snapshot bridge stats before submit so we can detect whether the
    # runtime that just received `submit()` is the one the bridge
    # wraps. If `received` doesn't tick after the run, the bridge is
    # registered on a DIFFERENT runtime (the plugin's own, which
    # nothing currently drives) and the desktop will never see the
    # events. This is the most common Pitfall #57 regression.
    try:
        from plugins.hermes_workflow.desktop_event_bridge import get_bridge
        bridge_before = dict(get_bridge().stats)
    except Exception as exc:
        bridge_before = {"bridge_probe_error": f"{type(exc).__name__}: {exc}"}
    ...
    # Log the post-submit state so the operator can correlate a
    # /api/workflows/run submission with the bridge's translation
    # counters in the same log timeline.
    try:
        from plugins.hermes_workflow.desktop_event_bridge import get_bridge
        bridge_after = dict(get_bridge().stats)
        _log.info(
            "workflow run submitted name=%s run_id=%s bridge.before=%s bridge.after=%s",
            name, run_id, bridge_before, bridge_after,
        )
    except Exception as exc:
        _log.info(
            "workflow run submitted name=%s run_id=%s (bridge probe failed: %s)",
            name, run_id, exc,
        )
...
# ----------------------------------------------------------------------------
# Workflow wire-up diagnostic — surfaces the runtime singleton, dispatcher
# chain, and desktop_event_bridge state to the renderer / operator so we can
# tell which link in the wire (Plugin → Runtime → Dispatcher → Bridge →
# tui_gateway → WebSocket → renderer) is broken. GET /api/workflows/diag
# returns a JSON snapshot. No side effects, no token required beyond the
# normal session token (kept for parity with the rest of the surface).
# ----------------------------------------------------------------------------
@app.get("/api/workflows/diag")
async def workflows_diag(request: Request):
    """Return a snapshot of the workflow wire-up state for debugging.

    Every field is the load-bearing answer to a "why doesn't the
    Hermes desktop's Workflows panel see my run?" question. Missing
    fields, mismatched runtimes, or a null dispatcher here are the
    difference between "run started" and "user sees nothing."
    """
    _require_token(request)
    diag: dict = {}

    # Path A: plugin's already-registered runtime (the one the
    # desktop_event_bridge is supposed to be wrapped around).
    try:
        from plugins.hermes_workflow import get_active_runtime
        plugin_rt = get_active_runtime()
    except Exception as exc:
        diag["plugin_get_active_runtime_error"] = f"{type(exc).__name__}: {exc}"
        plugin_rt = None
    diag["plugin_active_runtime_present"] = plugin_rt is not None

    # Path B: the singleton runtime the REST layer actually uses.
    # Per the seam in web_server.py:_workflow_runtime_singleton, this
    # should BE the plugin's runtime (so the bridge is in effect); if
    # it isn't, every REST-launched run is invisible to the desktop.
    rt = None
    try:
        rt = _workflow_runtime_singleton()
    except Exception as exc:
        diag["singleton_call_error"] = f"{type(exc).__name__}: {exc}"
    diag["singleton_runtime_present"] = rt is not None
    if plugin_rt is not None and rt is not None:
        diag["singleton_is_plugin_runtime"] = plugin_rt is rt
    else:
        diag["singleton_is_plugin_runtime"] = None

    # Dispatcher: the runtime's _dispatcher is the callback chain
    # the journal events flow through. If it's None, NOTHING is going
    # to the bridge. The runtime may wrap the existing dispatcher
    # (chained) so a non-None value is necessary but not sufficient.
    diag["runtime_dispatcher_set"] = bool(getattr(rt, "_dispatcher", None)) if rt is not None else False
    diag["runtime_dispatcher_type"] = (
        type(getattr(rt, "_dispatcher", None)).__name__
        if rt is not None and getattr(rt, "_dispatcher", None) is not None
        else None
    )

    # desktop_event_bridge registration state.
    try:
        from plugins.hermes_workflow.desktop_event_bridge import get_bridge
        bridge = get_bridge()
        stats = bridge.stats
        diag["bridge_present"] = True
        diag["bridge_stats"] = dict(stats)
        diag["bridge_registered_runtime_is_singleton"] = (
            bridge._wrapped_dispatcher is not None
            and getattr(rt, "_dispatcher", None) is not None
            and bridge._wrapped_dispatcher.__closure__ is not None
        )
    except Exception as exc:
        diag["bridge_present"] = False
        diag["bridge_error"] = f"{type(exc).__name__}: {exc}"

    # tui_gateway.server._emit reachability — does the emit function
    # this process would call actually exist? If the bridge is
    # registered but tui_gateway failed to import (the comment in
    # desktop_event_bridge.py:282 calls it out), stats.Translated
    # never goes anywhere.
    try:
        from tui_gateway.server import _emit as tgw_emit
        diag["tui_gateway_emit_importable"] = tgw_emit is not None
    except Exception as exc:
        diag["tui_gateway_emit_importable"] = False
        diag["tui_gateway_emit_error"] = f"{type(exc).__name__}: {exc}"

    # Active runs in the runtime — the in-memory record of every
    # workflow the user has launched. This is the source of truth the
    # /api/workflows/status endpoint reads; if it's empty after a
    # successful POST /api/workflows/run, the runtime singleton the
    # REST handler is using isn't the same one the run was submitted
    # to (the TTL cache evicted it, or the user is hitting a stale
    # backend).
    if rt is not None and hasattr(rt, "_runs"):
        try:
            runs = rt._runs
            diag["active_run_ids"] = sorted(runs.keys()) if isinstance(runs, dict) else []
        except Exception as exc:
            diag["active_runs_error"] = f"{type(exc).__name__}: {exc}"
    else:
        diag["active_run_ids"] = []
...
def start_server(
    ...
    ``ssh_session_token`` / ``ssh_owner_nonce`` are the process-local SSH
    bootstrap state minted by the desktop shell (read from a one-shot
    ``~/.hermes/desktop-ssh/<session>/...token`` file that cmd_dashboard
    already unlinked before calling us). Apply them eagerly so the WS
    auth middleware, /api/ssh/ownership probe, and the in-SPA token
    injection all see the same values the parent shell negotiated.
    """
    if ssh_session_token:
        _apply_ssh_session_token(ssh_session_token)
    if ssh_owner_nonce is not None:
...
# === Rule-1 catchup fix: upstream-only additions ===
# 80 missing top-level symbols (functions, classes, constants) appended in topo order so each symbol's dependencies come first. Source: upstream/main @ HEAD. Per user's 5-rule policy, these are
# upstream ADDITIONS that don't collide with our changes — they were correctly stayed-ours under the bulk-merge, but rule 1 forbids stomping them.


# --- _AUTO_ARCHIVE_CHECK_INTERVAL_S ---
...
def _serialize_field_value(field: DeclaredProviderField, value: Any) -> str:
...
# --- _provider_field_entry ---
def _provider_field_entry(field: DeclaredProviderField) -> Dict[str, Any]:
...
# --- _DASHBOARD_HEALTH_WINDOW_SECONDS ---
...
# --- _coerce_field_value ---
def _coerce_field_value(field: DeclaredProviderField, raw: str) -> Any:
...
# --- _DASHBOARD_SELFTEST_INTERVAL_SECONDS ---
_DASHBOARD_SELFTEST_INTERVAL_SECONDS = 60.0


# --- _dashboard_selftest_loop ---
...
# --- _read_field ---
def _read_field(field: DeclaredProviderField, sources: tuple, env: Dict[str, str]) -> Any:
...
# === Restored from upstream/main @hermes_cli/web_server.py (line 16391) ===
# Top-level binding lost by ours-wins catchup merge.
...
# --- _mcp_oauth_callback_url_from_base ---
...
def get_profiles_sessions_sidebar(
    ...
    return {
        "recents": {
            ...
            "total": len(recents_rows),
        },
        "cron": {"sessions": _window(cron_rows, cron_cap), "total": len(cron_rows)},
        "messaging": {
...
# === Restored from upstream/main @hermes_cli/web_server.py (line 300) ===
# Top-level binding lost by ours-wins catchup merge.
_SSH_OWNER_NONCE: Optional[str] = None

# === Restored from upstream/main @hermes_cli/web_server.py (line 5364) ===
# Top-level binding lost by ours-wins catchup merge.
_UNSET: Any = object()

# === Restored from upstream/main @hermes_cli/web_server.py (line 11712) ===
# Top-level binding lost by ours-wins catchup merge.
_last_auto_archive_check: Dict[str, float] = {}

# === Restored from upstream/main @hermes_cli/web_server.py (line 13051) ===
# Top-level binding lost by ours-wins catchup merge.
_mcp_oauth_flows: dict[str, "DashboardOAuthFlow"] = {}

# === Restored from upstream/main @hermes_cli/web_server.py (line 13053) ===
# Top-level binding lost by ours-wins catchup merge.
_mcp_oauth_transactions: dict[tuple[str, str], threading.Lock] = {}

# Register the SPA/no-frontend catch-all only after every API route. FastAPI
# resolves routes in declaration order; mounting this earlier swallowed routes
# restored later in this file, including /api/profiles/sessions/sidebar.
mount_spa(app)
