...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # MoA one-shot prefix: `-m moa:<preset>` (the `hermes chat -Q` path
        # and any caller that wants the MoA virtual provider without going
        # through `/moa`) must strip the prefix off ``self.model`` AND force
        # ``requested_provider="moa"`` so the existing
        # ``resolve_runtime_provider`` / ``agent_init`` MoA route runs
        # non-interactively. Without this, the raw ``moa:<preset>`` string
        # is sent to the real provider and rejected with a 401/400 "model
        # not supported" (#56828). The moa: prefix wins over an explicit
        # ``--provider`` flag.
        _moa_provider_override: Optional[str] = None
        if isinstance(model, str):
            _stripped = model.strip()
            if _stripped.lower().startswith("moa:"):
                _moa_provider_override = "moa"
                _after = _stripped.split(":", 1)[1].strip()
                model = _after or None
        ...
        # The ``moa:`` model-prefix override (set just above) takes precedence
        # over an explicit ``--provider`` so ``-m moa:<preset> --provider foo``
        # still routes to the MoA virtual provider (#56828).
    ...
    def process_command(self, command: str) -> bool:
        ...
        # $-prefixed skill/bundle chain at position 0: rewrite to /slash
        # form so the existing stacked-skill / bundle handlers carry the
        # rest. Mirrors the same affordance in TUI + Desktop. Mid-prose
        # $tokens are NOT rewritten here — they are inline references
        # handled by the completion adapters.
        if cmd_original.startswith("$"):
            from agent.skill_commands import rewrite_dollar_chain_to_slash
            rewritten, _dollar_refs = rewrite_dollar_chain_to_slash(cmd_original)
            if rewritten != cmd_original:
                cmd_original = rewritten
                cmd_lower = rewritten.lower()
...
# 10 missing top-level symbols (functions, classes, constants) appended in topo order so each symbol's dependencies come first. Source: upstream/main @ HEAD. Per user's 5-rule policy, these are
...
# === Restored from upstream/main @cli.py (line 126) ===
