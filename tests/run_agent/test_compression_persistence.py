...
class TestFlushAfterCompression:
    ...
    def test_rotation_child_session_flushes_full_compressed_transcript_with_markers(self):
        ...
        Atomic-publish contract (post #0ee8d4187): ``compress_context`` writes
        the compacted messages into the rotated child session as part of
        ``publish_compression_child`` (one transaction with end-parent +
        create-child). The child row count after ``compress_context`` MUST
        equal ``len(compressed)`` without any caller-side flush step.
        A subsequent ``rewrite_transcript`` (slash.compress) re-inserts
        ``compressed`` over those rows, but only after the caller merges
        any preserved tail — see ``gateway/slash_commands.py`` for the
        full-flow contract.
        """
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # relies on `compressed` containing no message that the atomic
            # publish filters for a reason INDEPENDENT of _db_persisted
            # (ephemeral scaffolding, synthetic recovery turns). Keep this
            # fixture free of such messages or the row count would
            # legitimately differ from len(compressed).
            messages = [
                {
                    "role": "user" if i % 2 == 0 else "assistant",
                    "content": f"message {i}",
            ...
            # Atomic publish already wrote the compacted transcript; assert
            # the row count matches without any caller-side flush.
