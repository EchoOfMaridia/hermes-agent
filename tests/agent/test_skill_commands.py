...
# ── $dollar-prefixed skill invocation (Plan: 2026-08-02-dollar-skill-invocation) ──
#
# The `$` token mirrors the inline `/skill-name` pattern: `$skill-a` is a
# reference the user drops into a prompt (mid-prose OR position-0), and
# `find_dollar_refs` resolves every reference to a canonical `/slug`. The
# `build_dollar_invocation_message` builder mirrors `build_stacked_skill_invocation_message`
# but accepts mixed skill/bundle refs.


class TestDollarSkillRefs:
    """`find_dollar_refs` resolves `$token` references in arbitrary text."""
    ...
    def _make_bundle(self, tmp_path, slug, skills, name=None):
        """Materialize a bundle YAML in the bundles dir."""
        from agent.skill_bundles import _bundles_dir
    ...
    def test_finds_position_zero_ref(self, tmp_path):
        from agent.skill_commands import find_dollar_refs
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert find_dollar_refs("$skill-a do the thing") == [("skill", "/skill-a")]

    def test_finds_mid_prose_ref(self, tmp_path):
        from agent.skill_commands import find_dollar_refs
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert find_dollar_refs("please clean $skill-b for me") == [("skill", "/skill-b")]

    def test_finds_multiple_refs_in_order(self, tmp_path):
        from agent.skill_commands import find_dollar_refs
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert find_dollar_refs("$skill-a $skill-b $skill-c run") == [
                ("skill", "/skill-a"),
                ("skill", "/skill-b"),
                ("skill", "/skill-c"),
            ]
    ...
    def test_dedupes_case_insensitively(self, tmp_path):
        ...
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert find_dollar_refs("$skill-a $SKILL-A $Skill_A run") == [("skill", "/skill-a")]

    def test_drops_unknown_ref_silently(self, tmp_path):
        from agent.skill_commands import find_dollar_refs
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert find_dollar_refs("$bogus-skill do the thing") == []

    def test_drops_disabled_skill_at_scan_time(self, tmp_path):
        """Globally-disabled skills don't appear in ``get_skill_commands()`` at
        scan time, so ``find_dollar_refs`` drops them automatically. This test
        confirms the gate is honored end-to-end without per-call re-check."""
        from agent.skill_commands import find_dollar_refs
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            # Mark skill-a as disabled via the helper the scan uses, then re-scan.
            with patch(
                "agent.skill_utils.get_disabled_skill_names",
                return_value={"skill-a"},
            ):
                scan_skill_commands()
                assert find_dollar_refs("$skill-a $skill-b do the thing") == [
                    ("skill", "/skill-b")
                ]

    def test_does_not_match_env_var_shaped_token(self, tmp_path):
        """`$PATH` and `$HOME` are env-var-shaped; must not match the parser."""
        from agent.skill_commands import find_dollar_refs
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            # All-uppercase token without letter-start is rejected by the regex.
            assert find_dollar_refs("run $PATH $HOME") == []

    def test_bundle_ref_resolves_when_both_collision(self, tmp_path):
        """Bundle names take precedence over skill names on collision."""
        from agent.skill_commands import find_dollar_refs
        from agent.skill_bundles import _bundles_dir
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            # Override env so the bundle dir is in tmp_path
            bundle_yaml = tmp_path / "skill-a.yaml"
            bundle_yaml.write_text(
                "name: skill-a\nskills:\n  - skill-b\n", encoding="utf-8"
            )
            with patch.dict(
                os.environ, {"HERMES_BUNDLES_DIR": str(tmp_path)}, clear=False
            ):
                # Need to clear cache since _bundles_dir resolves env at call time
                from agent.skill_bundles import _bundles_cache
                _bundles_cache.clear()
                refs = find_dollar_refs("$skill-a run")
        assert refs == [("bundle", "/skill-a")]
    ...
    def test_empty_text_returns_empty(self, tmp_path):
        ...
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert find_dollar_refs("") == []
            assert find_dollar_refs("just a plain prompt with no refs") == []


class TestBuildDollarInvocationMessage:
    """`build_dollar_invocation_message` reuses stacked-skill and bundle loaders."""
    ...
    def test_skills_only_uses_stacked_skill_builder(self, tmp_path):
        """Pure-skill `$a $b do XYZ` produces the same shape as the existing
        stacked-skill builder (so memory extractors, scaffold markers, and
        per-skill activation notes all keep working unchanged)."""
        from agent.skill_commands import build_dollar_invocation_message
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            result = build_dollar_invocation_message(
                "$skill-a $skill-b do the thing"
            )
        assert result is not None
        msg, loaded, missing = result
    ...
    def test_returns_none_when_no_known_refs(self, tmp_path):
        from agent.skill_commands import build_dollar_invocation_message
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            assert build_dollar_invocation_message("just plain text") is None
            assert build_dollar_invocation_message("$bogus do the thing") is None

    def test_unknown_refs_dropped_but_known_load(self, tmp_path):
        """Unknown `$bogus` is silently dropped; the rest still loads."""
        from agent.skill_commands import build_dollar_invocation_message
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            result = build_dollar_invocation_message(
                "$skill-a $bogus do the thing"
            )
        assert result is not None
        msg, loaded, missing = result
        assert loaded == ["skill-a"]
    ...
    def test_memory_extractor_recovers_instruction_from_dollar_turn(self, tmp_path):
        """The dollar-built scaffold reuses stacked-skill markers so
        memory providers keep recovering the user's instruction."""
        from agent.skill_commands import (
            build_dollar_invocation_message,
            extract_user_instruction_from_skill_message,
        )
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
    ...
    def test_bundle_ref_uses_bundle_builder(self, tmp_path):
        """`$bundle-name` loads the bundle via build_bundle_invocation_message,
        which already handles per-skill disabled gating and missing-skill notes."""
        from agent.skill_bundles import _bundles_cache
        from agent.skill_commands import build_dollar_invocation_message
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            bundle_yaml = tmp_path / "my-bundle.yaml"
            bundle_yaml.write_text(
                "name: my-bundle\nskills:\n  - skill-a\n  - skill-b\n",
                encoding="utf-8",
            )
