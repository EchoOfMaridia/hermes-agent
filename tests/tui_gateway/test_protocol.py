...
def test_command_dispatch_returns_stacked_skill_payload(server):
    """command.dispatch with stacked-skill arg (``/skill-a /skill-b do XYZ``) loads
    every leading skill into one scaffolded message — matches the classic CLI /
    gateway dispatch path so the TUI and Desktop stop being second-class citizens."""
    sid = "test-session"
    server._sessions[sid] = {"session_key": sid}
    ...
    fake_msg = (
        '[IMPORTANT: The user has invoked the "/skill-a /skill-b" stacked skill bundle, '
        ...
        "[Loaded as part of the stacked skill invocation \"skill-a\".]\n\nBody A."
    )

    # `split_stacked_skill_commands` calls `get_skill_commands` directly,
    # so patch BOTH the scan-time provider (for the dispatch's own lookup)
    # AND the live provider (for split_stacked_skill_commands to find /skill-b).
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
         patch("agent.skill_commands.get_skill_commands", return_value=fake_skills), \
         patch(
             "agent.skill_commands.build_stacked_skill_invocation_message",
             return_value=(fake_msg, ["skill-a", "skill-b"], []),
         ) as build_stacked, \
         patch("agent.skill_commands.build_skill_invocation_message") as build_single:
        resp = server.handle_request({
            "id": "r-stacked",
            "method": "command.dispatch",
            "params": {
                "name": "skill-a",
                "arg": "/skill-b do the thing",
                "session_id": sid,
            },
        })
    ...
    assert result["type"] == "send"
    assert result["message"] == fake_msg
    assert result["notice"] == "⚡ Loading 2 stacked skills: skill-a, skill-b"
    # Stacked-skill builder was invoked; single-skill builder was NOT.
    assert build_stacked.called
    assert not build_single.called


def test_command_dispatch_stacked_skill_single_arg_falls_back_to_single(server):
    """A command.dispatch with no extra ``/skill-x`` tokens in arg must still
    route through the single-skill path (existing behavior — regression guard)."""
    sid = "test-session"
    server._sessions[sid] = {"session_key": sid}

    fake_skills = {"/skill-a": {"name": "skill-a", "description": "A"}}
    fake_msg = "[IMPORTANT: The user has invoked the \"skill-a\" skill.]\n\nBody A."
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
         ...
         patch("agent.skill_commands.build_stacked_skill_invocation_message") as build_stacked:
        resp = server.handle_request({
            "id": "r-single",
            "method": "command.dispatch",
            "params": {
    ...
    assert result["type"] == "skill"
    assert result["message"] == fake_msg
    assert build_single.called
    assert not build_stacked.called
