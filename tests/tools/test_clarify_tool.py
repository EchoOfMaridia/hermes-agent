...
class TestClarifyContentKeyedChoices:
    """Dict-shaped choices with 'content' key must be unwrapped to user-facing text.

    MiniMax-M3 (and potentially other models) emit choices as
    ``[{"content": "option text"}]`` instead of bare strings. The
    _flatten_choice canonical key order was ``label → description →
    text → title`` — missing ``content``. Every content-keyed dict
    returned "" and was silently dropped, collapsing multi-choice
    clarifies to zero rows.

    Bug captured 2026-08-02: the LLM called
    ``clarify(question=..., choices=[{"content": "Defaults — ..."},
    {"content": "Tighter floor — ..."}, ...])`` against
    session 20260802_175713_c61494. All four choices returned ""
    from _flatten_choice, were filtered out, and the tool returned
    ``choices_offered: null`` — zero clickable rows on screen.
    """

    def test_flatten_unwraps_content_key(self):
        c = {"content": "Defaults — windowSize=50, stddevMultiplier=3.0"}
        assert _flatten_choice(c) == "Defaults — windowSize=50, stddevMultiplier=3.0"

    def test_flatten_content_vs_description_content_wins(self):
        # content comes before description in the key order
        c = {"content": "content value", "description": "description value"}
        assert _flatten_choice(c) == "content value"

    def test_flatten_label_takes_priority_over_content(self):
        # label still wins — it comes before content in the key order
        c = {"label": "label wins", "content": "content value"}
        assert _flatten_choice(c) == "label wins"

    def test_content_keyed_choices_reach_callback_as_clean_text(self):
        """End-to-end: content-keyed choices reach the UI as real text rows."""
        seen = []

        def cb(question, choices):
            seen.extend(choices or [])
            return choices[0] if choices else "picked"

        result = json.loads(
            clarify_tool(
                "Which config?",
                choices=[
                    {"content": "Defaults — windowSize=50, stddevMultiplier=3.0"},
                    {"content": "Tighter floor — stallMinSilenceMs=5000, maxStallRetries=2"},
                    {"content": "Match existing retry budget — maxStallRetries=5"},
                    {"content": "I will specify values directly"},
                ],
                callback=cb,
            )
        )
        assert len(seen) == 4, f"Expected 4 rows, got {seen}"
        assert all(isinstance(c, str) and c for c in result["choices_offered"])
        assert "Defaults — windowSize=50" in result["choices_offered"][0]
        assert result["user_response_mode"] == "selected"

    def test_content_keyed_choices_via_outer_list_unwrap(self):
        """Nested-list + content-keyed: _flatten_outer_lists then _flatten_choice."""
        seen = []
        # LLM sometimes emits choices=[[{"content": "a"}, {"content": "b"}]]
        from tools.clarify_tool import _flatten_outer_lists

        nested = [[{"content": "Option A"}, {"content": "Option B"}]]
        flattened = _flatten_outer_lists(nested)
        assert flattened == [{"content": "Option A"}, {"content": "Option B"}]
        # Each dict then goes through _flatten_choice
        assert _flatten_choice(flattened[0]) == "Option A"
        assert _flatten_choice(flattened[1]) == "Option B"
        ...
        result = json.loads(
            clarify_tool("Pick one", choices=nested, callback=cb)
        )
        assert len(result["choices_offered"]) == 2
