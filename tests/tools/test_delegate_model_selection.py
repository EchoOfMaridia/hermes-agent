...
(``opus``, ``gpt-5``, ``MiniMax-M2.7``, ``glm``) when fanning work out;
resolution reuses the shared ``model_switch`` pipeline so names are matched
leniently and the provider is resolved (not dictated).

Default behavior (since the model-selection default was flipped from off to
on): ``delegation.allow_model_selection`` defaults to ``true`` so the
``model`` field is exposed in the delegate_task schema and resolvable by
default. Users who want the legacy "subagents inherit the parent model"
contract can set ``delegation.allow_model_selection: false`` in
``config.yaml``.
...
class TestSchemaGating(unittest.TestCase):
    """The per-task `model` field is exposed by default; flag off hides it."""

    def test_flag_off_no_model_field(self):
        with patch(
            "tools.delegate_tool._load_config",
            return_value={"allow_model_selection": False},
        ):
    ...
    def test_default_on_exposes_model_field(self):
        """No config -> ``allow_model_selection`` defaults to True -> model field present."""
    ...
    def test_explicit_on_adds_model_field(self):
        with patch(
            "tools.delegate_tool._load_config",
...
class TestFlagGetter(unittest.TestCase):
    def test_default_on(self):
        """No config -> ``allow_model_selection`` defaults to True."""
        with patch("tools.delegate_tool._load_config", return_value={}):
