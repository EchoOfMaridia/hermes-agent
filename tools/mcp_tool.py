...
# Cap how much of a single server's stderr we re-surface in failure
# messages. A misbehaving server can write megabytes per second to stderr
# (e.g. a deprecation-warning storm from a Python import); embedding all
# of that into every retry-warning would flood the agent log. 4 KiB is
# enough to surface the leading traceback / import error / missing-module
# signal that operators actually need to diagnose a "Connection closed"
# without making the warning message itself unreadable.
_STDERR_TAIL_MAX_BYTES = 4096


def _read_stderr_tail_for(server_name: str, max_bytes: int = _STDERR_TAIL_MAX_BYTES) -> str:
    """Return the most recent stderr lines this server wrote to its log.

    The shared ``mcp-stderr.log`` interleaves output from every stdio MCP
    server across the lifetime of the profile, so this helper locates the
    last ``starting MCP server '<name>'`` marker and returns everything
    after it, truncated to ``max_bytes`` (best-effort: keeps the tail,
    not the head, so the leading traceback frame is preserved when the
    subprocess wrote less than ``max_bytes``).

    Returns an empty string if the log file cannot be read or no marker
    for this server exists yet. Never raises — failure to surface stderr
    must not make the failure warning itself raise.
    """
    try:
        from hermes_constants import get_hermes_home
        ...
        if not log_path.exists():
            return ""
        # Read the tail of the file; on large logs (multi-MB) this is
        # cheaper than loading the whole thing and finding the marker.
        with open(log_path, "rb") as handle:
            try:
                handle.seek(0, os.SEEK_END)
                size = handle.tell()
                if size > max_bytes * 4:
                    # 4× tail budget gives us room to find the marker
                    # without reading the whole file; a typical marker
                    # line is ~70 bytes.
                    handle.seek(size - max_bytes * 4)
                    handle.readline()  # discard the partial line we landed on
                else:
                    handle.seek(0)
            except OSError:
                return ""
            raw = handle.read().decode("utf-8", errors="replace")
        # Find the LAST header for this server. Header format:
        #     ===== [YYYY-MM-DD HH:MM:SS] starting MCP server '<name>' =====
        # Multiple servers can share the file, so scope to <name>.
        marker = f"starting MCP server '{server_name}'"
        last_marker = raw.rfind(marker)
        if last_marker == -1:
            return ""
        # Skip past the marker line itself (it ends with \n).
        after_marker = raw.index("\n", last_marker) + 1
        tail = raw[after_marker:]
        # Trim from the front if we exceeded the budget — preserve the
        # tail (the most recent output) so the leading traceback frame
        # is dropped first, but later context (the actual exception
        # line) survives.
...
def _enrich_subprocess_failure(server_name: str, root_exc: BaseException) -> str:
    """Format a subprocess-failure message with stderr tail + log path.

    Stdio MCP servers that crash on startup (broken cached venv, missing
    dependency, import error, syntax error) leave the operator staring at
    ``McpError: Connection closed`` with no clue WHY. The actual cause is
    in the subprocess's stderr, captured into the shared MCP stderr log.
    This helper formats the failure message so the agent log surfaces:

      1. The captured stderr tail (the import error / traceback / etc.).
      2. The path to the full log, for deeper inspection.

    Returns a multi-line string suitable for inclusion in a logger.warning
    call as the ``reason`` portion of the message.
    """
    tail = _read_stderr_tail_for(server_name)
    parts: list[str] = [f"{type(root_exc).__name__}: {root_exc}"]
    if tail:
        # Indent the tail so it stays visually grouped with the failure
        # line and is easier to copy out of the agent log.
        indented = "\n".join("    " + line for line in tail.splitlines() if line.strip())
        parts.append(f"subprocess stderr (tail):\n{indented}")
    try:
        from hermes_constants import get_hermes_home
        log_path = get_hermes_home() / "logs" / "mcp-stderr.log"
        parts.append(f"full stderr log: {log_path}")
    except Exception:
        pass
    return "\n".join(parts)
                ...
                if not self._ready.is_set():
                    ...
                    if failure_class == "permanent":
                        ...
                        logger.warning(
                            ...
                            self.name, _enrich_subprocess_failure(self.name, root),
                        )
                        self._error = exc
                        self._ready.set()
                        self._was_parked = True
                    ...
                    if initial_retries > _MAX_INITIAL_CONNECT_RETRIES:
                        logger.warning(
                            ...
                            _enrich_subprocess_failure(self.name, root),
                        )
                        self._error = exc
                        self._ready.set()
                        self._was_parked = True
