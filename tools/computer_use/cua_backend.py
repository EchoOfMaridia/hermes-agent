...
# Bound the portal pre-flight probe so a hung DBus call can't
# itself hang the start path. 500ms is generous for a session-bus
# NameHasOwner call (which is normally sub-millisecond) but safely
# bounds the worst-case latency.
_PORTAL_PROBE_TIMEOUT_S = 0.5


def _probe_xdg_desktop_portal(timeout: float = _PORTAL_PROBE_TIMEOUT_S):
    """Verify ``org.freedesktop.portal.Desktop`` is reachable on the session
    DBus. cua-driver 0.17.0 needs the portal's RemoteDesktop interface to
    wire libei input on Wayland; without it the MCP handshake wedges in
    ``mcp-initialize`` until the 30s ready-event timeout fires — leaving
    the operator with a nondiagnostic error.

    Returns the well-known bus name on success (currently unused by
    callers, but makes the probe composable for future gates). Raises
    ``RuntimeError`` with an actionable hint when the portal is not
    reachable, named distinctly from MCP startup errors so log
    classification stays clean.

    Only meaningful on Linux; returns a success sentinel unconditionally
    on macOS / Windows where cua-driver uses different input backends
    (CoreGraphics / UI Automation). The X11 / Windows sessions also
    return success — cua-driver falls back to AT-SPI + XTest there, so
    portal absence is non-fatal on those paths.
    """
    if sys.platform != "linux":
        return ("org.freedesktop.portal.Desktop",)  # non-Linux: no-op

    # X11-only sessions can use XTest/XSendEvent input without the
    # portal — portal is only required for native Wayland input via
    # libei. Detect Wayland and skip the probe otherwise so we don't
    # false-positive on X11 systems that intentionally omit the portal.
    if not os.environ.get("WAYLAND_DISPLAY"):
        return ("org.freedesktop.portal.Desktop",)  # X11 path: portal optional

    try:
        # dbus-send is the cheapest cross-process probe and inherits
        # the user's session bus automatically. ``NameHasOwner`` on the
        # session bus is the same call Hermes itself uses for portal
        # capability detection elsewhere.
        proc = subprocess.run(
            [
                "dbus-send",
                "--session",
                ...
                f"string:org.freedesktop.portal.Desktop",
            ],
            capture_output=True,
            text=True,
            timeout=timeout,
        ...
        if proc.returncode != 0:
            raise RuntimeError(
                f"dbus-send probe failed (rc={proc.returncode}): "
                f"{(proc.stderr or proc.stdout).strip()[:200]}"
            )
        # NameHasOwner returns ``boolean true`` on success, ``false``
        # otherwise. We treat both as evidence; the failure modes we
        # care about are rc != 0, hang, or missing binary — all of
        # which raise / fall through below.
        return ("org.freedesktop.portal.Desktop",)
    except FileNotFoundError:
        # No dbus-send on PATH — can't probe. Don't block the user;
        # the downstream cua-driver startup will fail anyway and the
        # existing 30s ready-event timeout still surfaces the wedge.
        return ("org.freedesktop.portal.Desktop",)
    except subprocess.TimeoutExpired:
        raise RuntimeError(
            "xdg-desktop-portal not reachable on the session bus "
            f"(dbus-send probe timed out after {timeout:.1f}s). "
            "cua-driver requires xdg-desktop-portal on Linux Wayland "
            "for input injection (libei RemoteDesktop backend). "
            "Install the portal for your compositor — e.g. "
            "`xdg-desktop-portal-kde` for KDE Plasma, "
            "`xdg-desktop-portal-gnome` for GNOME, "
            "`xdg-desktop-portal-hyprland` for Hyprland — and ensure "
            "the `xdg-desktop-portal` service is running. After "
            "installing, retry the computer_use call."
        )
    except Exception as exc:
        # Treat any other probe failure as "portal not reachable"
        # rather than swallowing it silently — the operator needs to
        # know the pre-flight couldn't confirm the portal.
        raise RuntimeError(
            f"xdg-desktop-portal pre-flight probe failed: {type(exc).__name__}: "
            f"{str(exc)[:200]}. cua-driver requires the portal on Linux "
            f"Wayland; install it for your compositor and ensure the "
...
class _CuaDriverSession:
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        # Portal pre-flight: cua-driver 0.17.0 requires
        # xdg-desktop-portal's RemoteDesktop interface on Linux Wayland
        # for input injection (libei). Without it, the MCP handshake
        # wedges in ``mcp-initialize`` and the 30s ready-event timeout
        # surfaces a nondiagnostic error. Detect the missing portal
        # up front in milliseconds with an actionable hint.
        try:
            _probe_xdg_desktop_portal()
...
# list_apps in favour of plain text content parts. Parse the text into
# the same shape _ingest_windows expects so the rest of the wrapper can
# stay transport-agnostic. Lines look like:
#   `- Calculator (window_id 1234, pid 997669, …)`
#   `- systemd (pid 1)`
# Anything we can't parse is skipped; the caller already handles a
# zero-window response by surfacing "no windows" to the user.
_LIST_WINDOWS_LINE_RE = re.compile(
    r"^\s*[-*]\s+"
    r"(?P<app>.+?)"
    r"\s*\(window_id\s+(?P<window_id>\d+)"
    r"(?:,\s*pid\s+(?P<pid>\d+))?"
    r"(?P<rest>[^)]*)\)"
    r"\s*$"
)
_LIST_APPS_LINE_RE = re.compile(
    r"^\s*[-*]\s+(?P<name>.+?)\s+\(pid\s+(?P<pid>\d+)\)\s*$"
)


def _parse_list_windows_text(text: str) -> List[Dict[str, Any]]:
    """Parse the cua-driver >= 0.19.0 list_windows text format.

    Returns dicts with the same keys as the structuredContent shape
    (app_name, pid, window_id, is_on_screen, title, z_index) so
    _ingest_windows can process them unchanged. z_index is unknown in
    the text format, so we leave it for _ingest_windows to default to 0.
    """
    parsed: List[Dict[str, Any]] = []
    for line in text.splitlines():
        m = _LIST_WINDOWS_LINE_RE.match(line)
        if not m:
            continue
        ...
        pid_raw = m.group("pid")
        if pid_raw is None:
            # No pid on the line — uncapturable, same rule as
            # _ingest_windows applies (skip).
            continue
        # The remainder inside the parens can carry "title=…", bounds, etc.
        # We don't currently consume it; future fields can be extracted
...
def _parse_list_apps_text(text: str) -> List[Dict[str, Any]]:
    """Parse the cua-driver >= 0.19.0 list_apps text format.

    Lines look like `- systemd (pid 1)`. Returns [{"name", "pid"}]
    dicts, matching the shape the existing list_apps text fallback
    produces.
    """
    parsed: List[Dict[str, Any]] = []
    for line in text.splitlines():
        m = _LIST_APPS_LINE_RE.match(line)
        if not m:
            continue
        parsed.append({"name": m.group("name").strip(), "pid": int(m.group("pid"))})
    return parsed
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def _call_capture_tool(self, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if out.get("isError") is True:
            # cua-driver >= 0.19.0 carries the failure reason in the text
            # content part of the CallToolResult, which _extract_tool_result
            # copies into `data`. The CLI-fallback path can also produce a
            # `data: None` shape with structuredContent.isError, so we widen
            # the message to anything useful that's present in the result.
            ...
            if not (isinstance(message, str) and message):
                ...
                if isinstance(sc, dict) and sc.get("error"):
                    message = str(sc["error"])
                else:
                    # Last-ditch: stringify whatever the wrapper saw, so the
                    # operator at least sees the failure mode instead of
                    # `capture failed: `.
                    message = f"isError=True data={out.get('data')!r} structuredContent={out.get('structuredContent')!r}"
            self._clear_active_target()
            raise RuntimeError(f"cua-driver {name} failed: {message}")
        return out
    ...
    def _load_windows(self) -> List[Dict[str, Any]]:
        ...
        # cua-driver >= 0.19.0 emits the window list as a text content part
        # (`- App (window_id N, pid P, …)`) rather than structuredContent.
        # Fall back to text parsing so the wrapper stays compatible across
        # both driver generations.
        if not raw_windows:
            text = out.get("data")
            if isinstance(text, str) and text:
                raw_windows = _parse_list_windows_text(text)
    ...
    def list_apps(self) -> List[Dict[str, Any]]:
        ...
        if isinstance(data, dict) and isinstance(data.get("apps"), list):
            return data["apps"]
        # cua-driver >= 0.19.0 emits a text list (`- systemd (pid 1)`)
        # under the summary line `Found N app(s): …`. Parse it; the
        # original regex below kept the older driver format working.
        if isinstance(data, str) and data:
            parsed_text = _parse_list_apps_text(data)
            if parsed_text:
                return parsed_text
