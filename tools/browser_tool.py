...
# 2026-06-29 11:23:36 incident: the original ``from hermes_constants import
# agent_browser_runnable, get_hermes_home`` line crashed every Discord turn
# cold-start when a transient stale ``hermes_constants`` bytecode view dropped
# ``agent_browser_runnable``. The gateway caught the ImportError in
# ``_handle_message_with_agent`` (run.py:9617) and returned a generic error
# bubble, but every subsequent turn hit the same crash until the bad bytecode
# was rebuilt -- effectively taking Discord offline from the user's
# perspective. We now resolve each name lazily inside a guarded block so a
# transient missing name cannot take the whole browser-tool import path down.
import importlib as _importlib_for_constants  # noqa: F401  # used below

_cached_hermes_constants_module: "object | None" = None


def _load_hermes_constants() -> "object | None":
    """Return the ``hermes_constants`` module, or ``None`` if not importable.

    Cached after first successful resolution. ``None`` callers must fall
    through to the safe defaults wired up below.
    """
    global _cached_hermes_constants_module
    if _cached_hermes_constants_module is not None:
        return _cached_hermes_constants_module
    try:
        _cached_hermes_constants_module = _importlib_for_constants.import_module(
            "hermes_constants"
        )
    except ImportError:
        _cached_hermes_constants_module = False  # sentinel: tried and failed
        return None
    return _cached_hermes_constants_module


def _resolve_hermes_constants_attr(
    name: str, fallback: "object | None" = None
) -> "object | None":
    module = _load_hermes_constants()
...
_hc_module = _load_hermes_constants()
get_hermes_home: "object" = (
    getattr(_hc_module, "get_hermes_home", None)
    if _hc_module is not None and _hc_module is not False
    else None
) or (lambda: None)


_agent_browser_runnable_resolved: "object | None" = None


def _resolve_agent_browser_runnable() -> "object | None":
    """Return the ``agent_browser_runnable`` callable from ``hermes_constants``.

    Cached after first successful resolution. ``None`` means the symbol is
    currently unavailable -- callers must treat that as "binary is NOT
    runnable" and fall through to the next resolution candidate rather than
    crash. This is the entire point: a transient stale-dependency view of
    ``hermes_constants`` (bad bytecode, partial install, mid-edit) cannot
    take down the Discord gateway anymore.
    """
    global _agent_browser_runnable_resolved
    if _agent_browser_runnable_resolved is not None:
        return _agent_browser_runnable_resolved

    candidate = _resolve_hermes_constants_attr("agent_browser_runnable")
    if not callable(candidate):
        _agent_browser_runnable_resolved = False
        return None
    _agent_browser_runnable_resolved = candidate
    return candidate


def agent_browser_runnable(path: "str | None") -> bool:
    """Runtime-safe wrapper that resolves ``hermes_constants.agent_browser_runnable``.

    Existing call sites use this exactly as they would the canonical
    implementation -- the function returns True iff the path is a
    runnable agent-browser CLI. When the symbol is unavailable
    (mid-rebuild, stale bytecode, partial install) this returns False for
    every input, the call sites fall through to the next resolution
    candidate, and the Discord turn completes successfully.
    """
    target = _resolve_agent_browser_runnable()
    if target is None or target is False:
        return False
    try:
        return bool(target(path))
    except Exception:
        # Mirror the spirit of the original implementation, which returned
        # False on any OSError / subprocess.TimeoutExpired: a flaky binary
        # must not crash the entire browser-tool resolution path.
        return False
...
try:
    ...
    # 2026-06-29 11:23:36 incident: a transient stale ``hermes_constants``
    # bytecode view dropped the symbol mid-rebuild and the Discord gateway
    # crashed every turn at this exact import line. Degrade to "not Termux"
    # -- the standard browser-install path covers every non-Termux host.
    def _is_termux_environment() -> bool:  # type: ignore[no-redef]
        return False
...
# 17 missing top-level symbols (functions, classes, constants) appended in topo order so each symbol's dependencies come first. Source: upstream/main @ HEAD. Per user's 5-rule policy, these are
...
# --- _headed_mode_resolved ---
_headed_mode_resolved = False


# --- _bare_task_id_for_session_key ---
...
# === Restored from upstream/main @tools/browser_tool.py (line 888) ===
# Top-level binding lost by ours-wins catchup merge.
_cached_headed_mode: Optional[bool] = None

# === Restored from upstream/main @tools/browser_tool.py (line 3526) ===
# Top-level binding lost by ours-wins catchup merge.
...
# === Restored from upstream/main @tools/browser_tool.py (line 3541) ===
# Top-level binding lost by ours-wins catchup merge.
