...
# ---------------------------------------------------------------------------
# Model-controlled-per-task field stripping
# ---------------------------------------------------------------------------
# Per-task dicts that the model supplies through ``delegate_task(tasks=[...])``
# may carry fields intended for non-delegation surfaces. Those fields must
# never reach ``delegate_task()`` itself, because allowing the model to drive
# them would re-introduce the model-controlled ACP-transport path that
# ``e4dbb67bf fix(security): remove model-controlled delegate ACP transport``
# closed. The stripper runs at the dispatcher boundary, so the rest of the
# delegate pipeline only ever sees inputs the parent agent actually
# authorized.
_MODEL_HIDDEN_TASK_FIELDS = frozenset({"acp_command", "acp_args"})


def _strip_model_hidden_task_fields(tasks: Any) -> Any:
    """Return ``tasks`` with model-hidden keys removed from each task entry.

    Pure function. Inputs that are not a list of dicts are returned
    unchanged so callers can pass through ``None`` / strings / single dicts
    without first normalising. When the input list contains no hidden keys
    on any entry, the original object is returned to preserve identity for
    callers that compare before-and-after references.

    @param tasks The per-task payload from a model-supplied ``delegate_task``
        call, may be ``None`` or any other non-list value.
    @return The same payload with keys in ``_MODEL_HIDDEN_TASK_FIELDS``
        removed from each dict entry, or the original value when nothing
        needed to change.
    """
...
# ---------------------------------------------------------------------------
# Subagent endpoint inheritance
# ---------------------------------------------------------------------------
# ``parent_agent.base_url`` can still carry a leftover URL from an earlier
# config (for example, OpenRouter) while the live OpenAI client stored in
# ``parent_agent._client_kwargs`` already points at a different endpoint (for
# example, local Ollama). Subagents must inherit the endpoint the parent is
# actually calling, otherwise they 401 against the stale URL with the
# parent's local key. Behaviour pinned by
# ``25b734845 fix(delegate): inherit subagent endpoint from parent active client``
# and ``2608f78b9 test(delegate): cover stale parent base_url inheritance``.
def _normalized_runtime_url(value: Any) -> str:
    """Return ``value`` as a stripped runtime URL string for equality checks.

    @param value Any URL-like value or ``None``.
    @return A string with leading/trailing whitespace stripped and any
        trailing slash removed, suitable for stable comparison.
    """
    return str(value or "").strip().rstrip("/")


def _inherit_parent_base_url(parent_agent: Any, fallback_base_url: Optional[str]) -> Optional[str]:
    """Return the base URL the parent is actually calling.

    Prefers the URL inside ``parent_agent._client_kwargs['base_url']`` when it
    differs from the surface attribute and is a real http(s) URL. Falls back
    to ``parent_agent.client.base_url`` when available. Returns the input
    fallback (or ``None``) when neither override is set.

    @param parent_agent The parent ``AIAgent`` instance.
    @param fallback_base_url The base URL the caller would use as a default.
    @return The effective base URL for the child subagent.
    """
...
def _get_allow_model_selection() -> bool:
    ...
    Config key: delegation.allow_model_selection (bool, default True).
    On by default so the model can route work to alternative models
    (``opus``, ``gpt-5``, ``MiniMax-M2.7``, etc.) when it makes sense — fan out
    a code review across several models, dispatch a slow research task to a
    cheaper model, etc. Set ``delegation.allow_model_selection: false`` in
    config.yaml to force every child to inherit the parent model (useful for
    users who want strict parity with the historical behavior).
    """
    cfg = _load_config()
    return is_truthy_value(cfg.get("allow_model_selection"), default=True)
...
def _resolve_task_model_creds(model_name: str, parent_agent, base_creds: dict) -> dict:
    ...
    The agent names a model ("opus", "gpt-5", "glm") and we resolve it leniently
    — provider is *resolved, not dictated*. Reuses the existing aggregator-aware
    ``hermes_cli.model_switch.switch_model()`` pipeline (the same chain the
...
# MiniMax-M3 rate-limits parallel subagent fan-out at ~200 req/min. MiniMax-M2.7
# on the same provider family is rated ~500 req/min. When a batch exceeds the
# configured threshold (default 2), swap the inherited M3 model for M2.7 so the
# parallel children don't trip the M3 rate limiter. Disable with
# delegation.minimax_m27_batch_threshold: 0, or change the target model with
# delegation.minimax_m27_fallback_model.
_DEFAULT_MINIMAX_M27_THRESHOLD = 2
_DEFAULT_MINIMAX_M27_FALLBACK_MODEL = "MiniMax-M2.7"
_MINIMAX_M27_PROVIDERS = frozenset({"minimax", "minimax-cn", "minimax-oauth"})


def _get_minimax_m27_threshold() -> int:
    """Read delegation.minimax_m27_batch_threshold from config (default 2).

    Floor 0 (0 disables the swap). Negative values coerce to 0.
    """
    cfg = _load_config()
    val = cfg.get("minimax_m27_batch_threshold", _DEFAULT_MINIMAX_M27_THRESHOLD)
    try:
        threshold = int(val)
    except (TypeError, ValueError):
        logger.debug("minimax_m27_batch_threshold=%r is not an int; using default", val)
        return _DEFAULT_MINIMAX_M27_THRESHOLD
    return max(0, threshold)


def _get_minimax_m27_fallback_model() -> str:
    """Read delegation.minimax_m27_fallback_model from config (default MiniMax-M2.7)."""
    cfg = _load_config()
    raw = cfg.get("minimax_m27_fallback_model", _DEFAULT_MINIMAX_M27_FALLBACK_MODEL)
    model = str(raw or "").strip()
    return model or _DEFAULT_MINIMAX_M27_FALLBACK_MODEL
...
# Subagents now default to 100 iterations (was 50). Real research, multi-file
# reviews, and TDD-with-many-failing-tests scenarios were hitting the old
# ceiling mid-task; legitimate work runs longer than the safety floor assumed.
# The per-call max_iterations kwarg is still ignored (config is authoritative),
# but the new default lifts the ceiling for everyone. Raise further via
# delegation.max_iterations in config.yaml.
...
def delegate_task(
    ...
    # Batch fan-out on MiniMax M3 hits the 200 req/min rate ceiling once you
    # fan out past a couple of children. MiniMax-M2.7 is rated 500 req/min
    # on the same provider family, so large batches route through it instead.
    # Threshold defaults to 2 (i.e. n_tasks > 2 swaps to M2.7). Set
    # delegation.minimax_m27_batch_threshold: 0 to disable, or to a higher
    # number to require a larger batch before swapping. The override only
    # fires when (a) the parent is on a MiniMax provider, (b) the user has
    # not explicitly set delegation.model/delegation.provider in config, and
    # (c) the child is otherwise inheriting the parent's M3 model — never
    # touches an explicit user choice.
    if n_tasks > _get_minimax_m27_threshold() and creds.get("model") is None and creds.get("provider") is None:
        parent_provider = str(getattr(parent_agent, "provider", "") or "").strip().lower()
        if parent_provider in _MINIMAX_M27_PROVIDERS:
            fallback_model = _get_minimax_m27_fallback_model()
            creds["model"] = fallback_model
            logger.info(
                "delegate_task: batch fan-out of %d tasks on %s — swapping child model "
                "from inherited %s to %s (higher rate ceiling for parallel work). "
                "Override via delegation.model or delegation.minimax_m27_batch_threshold.",
                n_tasks, parent_provider, getattr(parent_agent, "model", "?"), fallback_model,
            )
    ...
    try:
        ...
        for i, t in enumerate(task_list):
            ...
            # default creds. The flag-off path is byte-identical to the prior
            # behavior — no schema field, no extra resolution call.
            task_creds = creds
...
def _build_dynamic_schema_overrides() -> dict:
    ...
    # Per-task model selection (gated behind delegation.allow_model_selection).
    # Only advertise the `model` field to the model when the user has opted in;
    # otherwise the schema is byte-identical to the prior behavior, preserving
    # the "subagents inherit the parent model" contract. The flag is a stable
    # per-session config value, so toggling it between sessions (not
...
# 12 missing top-level symbols (functions, classes, constants) appended in topo order so each symbol's dependencies come first. Source: upstream/main @ HEAD. Per user's 5-rule policy, these are
