...
def dispatch_async_delegation(
    *,
    ...
    # Capacity check, record insert, and durable persist under ONE lock hold —
    # checking active_count() separately would let two concurrent dispatches
    # (e.g. from different gateway sessions) both pass the check and exceed
    # the cap, and calling _persist_dispatch after the lock is released would
    # leave an in-memory record at status="running" with no worker thread
    # ever created if the SQLite write raises (lock contention, IOError,
    # schema drift, …). The latter was the user's "insta crashes after one
    ...
    with _records_lock:
        ...
        try:
            ...
            # Roll the in-memory insert back so capacity count and durable
            # state stay aligned. _persist_dispatch has no side effects
            # outside this record, so popping the dict entry is enough.
            _records.pop(delegation_id, None)
            logger.warning(
                "Async delegation %s dispatch persist failed; "
                "releasing capacity slot: %s",
                delegation_id,
                exc,
            )
            return {
    ...
    *,
    ...
    with _records_lock:
        ...
        try:
            ...
            # Roll the in-memory insert back so the capacity count and
            # durable state stay aligned. See dispatch_async_delegation
            # for the same fix and its history.
            _records.pop(delegation_id, None)
            logger.warning(
                "Async delegation batch %s dispatch persist failed; "
                "releasing capacity slot: %s",
                delegation_id,
                exc,
            )
...
def _stale_monitor_loop() -> None:
    ...
    Orphan backstop: a ``running`` record with no ``progress_fn`` (e.g. one
    that escaped the dispatch-time persist rollback, or whose worker
    thread died without calling ``_finalize``) is otherwise immortal from
    this loop's perspective. To prevent it from permanently shrinking the
    capacity budget — the exact failure mode the user hit in
    ``session 20260727_094104_9b676b`` (2026-07-27), where multiple
    records were stuck at ``running`` in ``_records`` after their durable
    row had already moved on — we flip those records to ``stalling`` after
    the grace window elapses (measured from ``dispatched_at``) and let the
    existing ``_finalize_stalled`` path free the slot.
    """
    while not _monitor_stop.wait(_STALE_CHECK_INTERVAL):
        ...
        orphan_expired: List[str] = []  # progress_fn-less running records past grace
        ...
        with _records_lock:
            for record in _records.values():
                ...
                if progress_fn is None:
                    # Orphan backstop: progress_fn-less records have no
                    # heartbeat to sample, so measure quietness from the
                    # dispatch time. Past the grace window, flip straight
                    # to expired (no interrupt_fn means there's nothing
                    # to signal — the worker thread is already gone).
                    any_monitorable = True
                    dispatched_at = record.get("dispatched_at") or now
                    if now - dispatched_at >= _STALL_GRACE_SECONDS:
                        orphan_expired.append(record["delegation_id"])
                    continue
                any_monitorable = True
                try:
        ...
        for delegation_id in orphan_expired:
            logger.warning(
                "Async delegation %s has no progress_fn and has been "
                "running for >%.0fs — force-finalizing as orphan "
                "(see session 20260727_094104_9b676b regression)",
                delegation_id,
