...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _run_simple_slash(
        ...
        # response. If the auth check or anything else delays us past that,
        # `defer()` raises discord.NotFound (10062 — Unknown Interaction)
        # and the rest of the command would also blow up. Catch it, log
        # the exact failure, mark the interaction as terminal, and let
        # handle_message still run so the user gets *some* response via
        # the gateway instead of a silent death.
        defer_failed = False
        try:
            await interaction.response.defer(ephemeral=True)
        except discord.NotFound as e:
            # 10062 — token already expired. No followup/edit will work.
            logger.warning(
                "Discord interaction %s for command '%s' expired before defer "
                "(NotFound 10062): %s — dispatching without interaction response",
            ...
            # 429/5xx — defer failed but the interaction is still live.
            # Subsequent followup/edit/delete may still work, so don't
            # mark it terminal — just log and continue.
            logger.warning(
                "Discord interaction %s for command '%s' defer() failed: %s",
                getattr(interaction, "id", "?"),
                command_text,
                e,
            )
        ...
        if defer_failed:
            # Interaction token is dead — no further Discord calls will work.
            # The gateway response will arrive as a plain channel message.
            return
        try:
            if followup_msg:
    ...
    async def _handle_thread_create_slash(
        ...
        if not await self._check_slash_authorization(interaction, "/thread"):
            return
        # Same defer-failure guard as _run_simple_slash — 10062 kills the
        # entire command if we don't catch it.
        defer_failed = False
        try:
            await interaction.response.defer(ephemeral=True)
        except discord.NotFound as e:
            logger.warning(
                "Discord interaction %s for /thread expired before defer "
                "(NotFound 10062): %s",
                getattr(interaction, "id", "?"),
                e,
            )
            ...
            logger.warning(
                "Discord interaction %s for /thread defer() failed: %s",
                getattr(interaction, "id", "?"),
                e,
            )
        ...
        if defer_failed:
            # Interaction token is dead — no followup will work.
            if not result.get("success"):
                logger.warning(
                    "Thread creation failed for /thread and interaction "
                    "token is expired — cannot report to user"
                )
            return
    ...
    async def _handle_bare_mention_ack(self, message: Any) -> bool:
        """Send a short in-channel ack for a bare ``@bot`` ping.

        Returns ``True`` when the ack was sent (or attempted) successfully.
        Returns ``False`` when no channel is available — callers should still
        drop the message in that case.

        The bare-mention filter (see ``on_message``) deliberately drops
        mention-only messages with no other text to avoid spawning an
        empty-text agent turn. Without an ack that drop looks identical to
        "the bot is dead" from the user's perspective. A short 👋 reply is
        enough to confirm liveness without breaking the empty-turn guarantee.

        Failure is logged at DEBUG and swallowed — a missing ack must never
        crash the message dispatcher or fall through into an agent turn.
        """
        logger.info(
            "[%s] Bare mention-only ping from %s in %s — sending ack",
            self.name,
            getattr(
                message.author,
                "display_name",
        ...
        try:
            ...
            logger.debug(
                "[%s] bare-mention ack send failed (non-fatal): %s",
                self.name,
                ack_err,
            )
            return False
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
        ...
        if (not event_text or not event_text.strip()) and not _channel_context:
            ...
            if (
                ...
                await self._handle_bare_mention_ack(message)
                # Bare mention is dropped in both ack-sent and ack-failed
                # cases — the helper logs either way, and we never spawn an
                # empty-text agent turn for a mention with no other content.
                return
...
# 9 missing top-level symbols (functions, classes, constants) appended in topo order so each symbol's dependencies come first. Source: upstream/main @ HEAD. Per user's 5-rule policy, these are
