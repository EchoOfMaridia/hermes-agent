    ...
    def run():
        ...
        try:
            ...
            if status == "complete" and isinstance(raw, str) and raw.strip():
                try:
                    from hermes_cli.goals import GoalManager, DEFAULT_MAX_TURNS as _GOAL_DEF_TURNS

                    sid_key = session.get("session_key") or ""
                    if sid_key:
                        try:
                            goals_cfg = _load_cfg().get("goals") or {}
                            goal_max_turns = int(goals_cfg.get("max_turns", _GOAL_DEF_TURNS) or _GOAL_DEF_TURNS)
                        except Exception:
                            goal_max_turns = _GOAL_DEF_TURNS
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        key = f"/{name}"
        if key in cmds:
            # Stacked-skill invocations: `name=skill-a` with `arg=/skill-b /skill-c do XYZ`
            # loads every leading skill into one scaffolded message. Mirrors
            # `cli.py:9034` and `gateway/run.py:10474` so the TUI and Desktop
            # are not second-class citizens when the user types the
            # `/a /b` stacked form (or the `$a $b` dollar form, which the
            # desktop rewrites to `/a /b` before reaching this branch).
            extra_keys: list[str] = []
            try:
                extra_keys, stacked_instruction = split_stacked_skill_commands(arg or "")
            except Exception:
                stacked_instruction = arg or ""
            if extra_keys:
                ...
                if stacked_result is not None:
                    ...
                    notice = (
                        f"⚡ Loading {len(stacked_loaded)} stacked skills: "
                        f"{', '.join(stacked_loaded)}"
                    )
                    if stacked_missing:
                        notice += (
                            f"\nSkipped missing skills: {', '.join(stacked_missing)}"
                        )
                    return _ok(
                        rid,
                        {
                ...
                # Stacked builder failed entirely — fall through to single
    ...
    if name == "goal":
        ...
        try:
            from hermes_cli.goals import (
                GoalManager,
                DEFAULT_MAX_TURNS as _GOAL_DEF_TURNS,
            )
        except Exception as exc:
        ...
        try:
            goals_cfg = _load_cfg().get("goals") or {}
            max_turns = int(goals_cfg.get("max_turns", _GOAL_DEF_TURNS) or _GOAL_DEF_TURNS)
        except Exception:
            max_turns = _GOAL_DEF_TURNS
...
# 108 missing top-level symbols (functions, classes, constants) appended in topo order so each symbol's dependencies come first. Source: upstream/main @ HEAD. Per user's 5-rule policy, these are
...
# --- _AUTO_CONTINUE_MAX_ATTEMPTS_DEFAULT ---
...
# --- _DASHBOARD_TURN_ISOLATION_DEFAULT ---
...
# === Restored from upstream/main @tui_gateway/server.py (line 1305) ===
# Top-level binding lost by ours-wins catchup merge.
_live_transports: set[Transport] = set()

# === Restored from upstream/main @tui_gateway/server.py (line 2858) ===
# Top-level binding lost by ours-wins catchup merge.
_last_skin_sig: tuple[str, float | None] | None = None

# === Restored from upstream/main @tui_gateway/server.py (line 14369) ===
# Top-level binding lost by ours-wins catchup merge.
_DIR_EXISTS_CACHE: dict[str, bool] = {}

# === Restored from upstream/main @tui_gateway/server.py (line 17214) ===
# Top-level binding lost by ours-wins catchup merge.
