...
class AIAgent:
    ...
    def _compress_context(
        ...
        *,
        ...
        ``defer_context_engine_notification=True`` is passed by host
        handlers (the ``/compress`` slash command, the TUI
        ``session.compress`` method) that own the durable commit and want
        the boundary hook to fire only after the host transaction
        commits. ``finalize_context_engine_compression_notification(agent,
        committed=True/False)`` then drains or discards the queued
        notification.
        """
